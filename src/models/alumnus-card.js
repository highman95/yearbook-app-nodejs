...
const { BadRequestError, DatabaseError } = require('../utils/http-errors')

module.exports = {
    async addOne(alumniId, tagNo, institutionId, divisionId, subDivisionId) {
        if (!!await this.find(alumniId, institutionId)) throw new BadRequestError('This profile already exists')
        if (!!await this.findByTagNo(tagNo, institutionId)) throw new BadRequestError('This registration number already exists')

        try {
            const input = [alumniId, tagNo, institutionId, divisionId, subDivisionId, '', '']
